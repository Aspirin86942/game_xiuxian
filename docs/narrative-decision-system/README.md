...
- 当前第一优先任务已经从“把第三卷《魔道入侵》正式入册并接入运行时”推进到“把第四卷《风起海外》正式入册并接入运行时”。
- 第四卷当前目标是：在不回退第三卷卷末边界的前提下，把 `21 初入星海` 之后的乱星海主线固定成 8 章闭环，并把 `23_mocaihuan_return` 提升为第四卷核心章节。
- `20 再别天南` 继续固定为第三卷卷末出口；issue #11 中“再别天南 / 并肩之后”的语义，只能在第四卷中被读取、延展和再确认，不回退第三卷已锁定的章节边界。
- `24 重返天南` 与 `25 化神飞升` 继续保留为下一卷入口资产，不在第四卷本轮被提前并入核心 8 章。
